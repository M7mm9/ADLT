...
if UserName in UserName_list:
    ...
    if Today <= FinishDate:
        ...
        st.sidebar.write('--------------')
        ItemGroup = st.sidebar.selectbox('Item Group', (
            'All Items', '908 Group', 'CA Group', 'CK Group', 'CT Group', 'EP Group', 'Other Items', 'Test'))
        st.sidebar.write('--------------')
        Period = st.sidebar.select_slider('Forecasting Horizon', range(1, 13))

        Path = rf'C:\Users\maham\Desktop\KFUPM\Senior Project\ARIMA Data\{Year}\{ItemGroup}\Orders.csv'
        ...
        Path = rf'C:\Users\maham\Desktop\KFUPM\Senior Project\ARIMA Data\{Year}\{ItemGroup}\Quantitya.csv'
        Data1 = pd.read_csv(Path)
